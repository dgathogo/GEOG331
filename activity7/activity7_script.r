...
dirR <- '~/github/envdatascience/oneida'
...
#legend
legend("bottomleft", paste(landclass$landcover),
       fill=landclass$cols ,bty="n", cex=0.5)
#add title
mtext("Random Forest", side=3,cex=1, line=-5)

#neural network
plot(nnet_prediction,
     breaks=seq(0,6), 
     col=landclass$cols ,
     legend=FALSE, axes=FALSE, cex.axis=0.5)
#add legend
legend("bottomleft", paste(landclass$landcover),
       fill=landclass$cols, bty="n", cex=0.5)   
#add title
mtext("Neural network", side=3,cex=1, line=-5)
...
freq(rf_prediction)
#Question 5
merged_prediction <- overlay(nnet_prediction, rf_prediction,
                             fun=function(r1,r2){return (abs(r1-r2))})
plot(merged_prediction)
mtext("Difference between Neural Network and Random Forest", side=3,cex=1, line=-5)
